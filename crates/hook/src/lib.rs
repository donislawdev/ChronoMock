//! Chrono Mock injected hook (Stage 3): substitute the FULL set of wall-clock
//! channels, consistently.
//!
//! On `DLL_PROCESS_ATTACH` this opens the session control memory (`Local\ChronoCtl`),
//! installs a MinHook detour on every wall-clock export listed in
//! `chrono_ctl::CHANNELS`, and records each covered channel in the control block.
//! Each detour returns `a_fake + (quit_now - a_real) * multiplier` (Stage 3 uses
//! multiplier 1), anchored on `QueryUnbiasedInterruptTime` (ADR-5). The UTC channels
//! return that instant directly; `GetLocalTime` returns it shifted back into the
//! session zone by `tz_bias`.
//!
//! ABSOLUTE, not delta: a detour computes the fake instant from the anchor and never
//! calls another wall export's original. So there is no cross-channel re-entrancy and
//! no double-shift, and hence no thread-local re-entrancy guard - the spike's E2 guard
//! was an artifact of an earlier delta design (`original + delta`) and does not apply.
//!
//! Out of scope here (later slices): the session zone-info functions
//! (`GetTimeZoneInformation` / `GetDynamicTimeZoneInformation`), the duration axis
//! (`GetTickCount64` / `QUIT` scaling; `QueryPerformanceCounter` deliberately excluded
//! per ADR-2), and child-process inheritance (`CreateProcessW`).
...
use chrono_ctl::{
    ...
    IDX_GLT, IDX_GST, IDX_GSTAFT, IDX_GSTPAFT, IDX_NTQST,
};
use minhook::MinHook;
