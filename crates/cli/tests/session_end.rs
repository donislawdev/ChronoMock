...
/// Samples every few milliseconds for `SAMPLE_MS` of real time and writes, space separated: the tick
/// rate over the first real second, the tick rate over the last real second, and the largest step back
/// seen on the tick count, the interrupt time and QPC (0 when an axis never went back).
#[test]
...
fn probe_samples_the_duration_axes_past_the_session_end() {
    ...
    let first_tick = last[0];
    let mut head_rate = None;
    ...
    loop {
        ...
        last = reading;
        if head_rate.is_none() && now >= 1_000.0 {
            head_rate = Some((reading[0] - first_tick) / now);
        }
...
/// What the probe wrote: `[head rate, tail rate, step back on tick, on QUIT, on QPC]`.
fn measured(file: &std::path::Path) -> Option<[f64; 5]> {
...
fn an_application_left_running_keeps_its_duration_axes_moving_forward() {
    ...
    let [head, tail, ..] = measured(&control).expect("the probe wrote nothing without a session");
    assert!(head < 2.0 && tail < 2.0, "without a session the tick count moved x{head} then x{tail}, so this probe cannot tell");
    ...
    let Some([head, tail, tick_back, quit_back, qpc_back]) = measured(&session) else {
        panic!("the probe wrote nothing under the session. {}", context());
    };
    assert!(head > 10.0, "the tick count moved x{head} during the session, so the session never scaled it. {}", context());
