...
/// wire. The error side carries the exit code rather than a message, because every failure on this
/// path is a usage or catalogue error and not a substitution verdict - the text is printed next to
/// the branch that knows what went wrong.
///
/// Returning the wire shape instead of five loose values is the point. The terminal report and the
/// evidence file describe the session by reading this same value, so neither can describe a mode or
/// a moment other than the one that was sent (untouchable rule 4).
pub(super) fn resolve_time_spec(ra: &RunArgs, now_bias: i32) -> Result<TimeSpec, i32> {
...
mod tests {
    ...
    fn an_absolute_at_carries_the_session_zone_onto_the_wire() {
        ...
        let spec = resolve_time_spec(&ra, 120).expect("an absolute moment needs neither disk nor clock");
    ...
    fn the_mode_flags_reach_the_wire_unchanged() {
        ...
        .unwrap();
        let spec = resolve_time_spec(&ra, 0).unwrap();
        ...
        let spec = resolve_time_spec(&plain, 0).unwrap();
